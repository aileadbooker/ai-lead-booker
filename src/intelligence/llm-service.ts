...
` : '';
            ...
            const systemPrompt = `You are an expert sales development representative (SDR) for ${config.businessName}.
Your goal is to book a meeting with the lead.
You are reachable at ${config.businessName} (email: ${config.smtp.user}).
...
Task: Generate the next move and email draft.`;
     ...
     */
    async generateLeadList(niche: string, count: number = 5): Promise<Array<{ name: string, company: string, email: string }>> {
