import { Lead, LLMDecision, BusinessConfig, KnowledgeBaseEntry } from '../types';
...
export class QualificationEngine {
     ...
     */
    async qualifyLead(lead: Lead, isFollowUp: boolean = false): Promise<LLMDecision> {
        // Load dependencies
     ...
     */
    private generateMissingFieldsMessage(missingFields: string[]): string {
     ...
     */
    private async getKnowledgeBase(): Promise<KnowledgeBaseEntry[]> {
        const result = await db.query(
            `SELECT * FROM knowledge_base ORDER BY type, key`
        );
     ...
     */
    private async getConversationHistory(leadId: string): Promise<string[]> {
        // Get messages
       ...
       LIMIT 10`,
        ...
        const history: { timestamp: string, text: string }[] = [];
...
export default new QualificationEngine();
