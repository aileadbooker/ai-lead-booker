...
class EmailService {
    ...
    async processMessage(message) {
        ...
        await tracker_1.analyticsTracker.track('reply_received', lead.id);
        return lead;
    }
    /**
    ...
    async getTransporter() {
        ...
        try {
            const res = await database_1.default.query('SELECT email, access_token, refresh_token, google_id, google_app_password, google_account_email FROM users LIMIT 1');
     ...
     * Send email via SMTP
     */
    async sendEmail(lead, subject, body, threadInfo, attachments // Support attachments (like ICS)
    ) {
        if (!config_1.config.enableRealEmail) {
            console.log(`[MOCK EMAIL] To: ${lead.email}, Subject: ${subject}`);
            await tracker_1.analyticsTracker.track('email_sent', lead.id);
        ...
        try {
            ...
            const res = await database_1.default.query('SELECT email, google_account_email FROM users LIMIT 1');
            const user = res.rows[0] || {};
            const senderEmail = user.google_account_email || config_1.config.gmailUserEmail || config_1.config.smtp.user;
            const mailOptions = {
