...
class SummaryGenerator {
    constructor() {
        this.tracker = new tracker_1.AnalyticsTracker();
    }
    /**
    ...
    async generateAndSend() {
        ...
        const stats = await this.tracker.getSummary(startDate.toISOString(), endDate.toISOString());
        // 2. Get recent bookings (last 24h)
        const bookings = await database_1.default.query(`SELECT l.email, l.name, l.company, m.body 
             FROM leads l
             JOIN messages m ON l.id = m.lead_id
             WHERE l.status = 'booked' 
             AND l.updated_at >= $1
             AND m.direction = 'inbound'
             ORDER BY l.updated_at DESC`, [startDate.toISOString()]);
        // 3. Generate HTML
        ...
        await email_service_1.default.sendAdminReport(`Daily Summary - ${dateStr}`, html);
    }
    buildHtml(stats, bookings, date) {
