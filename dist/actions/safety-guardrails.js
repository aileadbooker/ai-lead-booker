...
exports.SafetyGuardrails = void 0;
...
class SafetyGuardrails {
     ...
     */
    async checkSafetyRules(lead, action) {
    ...
    async checkRateLimit(leadId) {
        const now = new Date();
        // Check daily limit
        const dailyResult = await database_1.default.query(`SELECT count, reset_at FROM rate_limits 
       WHERE lead_id = $1 AND period = 'daily' AND reset_at > $2`, [leadId, now.toISOString()]);
        ...
        // Check weekly limit
        const weeklyResult = await database_1.default.query(`SELECT count, reset_at FROM rate_limits 
       WHERE lead_id = $1 AND period = 'weekly' AND reset_at > $2`, [leadId, now.toISOString()]);
     ...
     */
    async incrementRateLimit(leadId) {
        const now = new Date();
        ...
        await database_1.default.query(`INSERT INTO rate_limits (id, lead_id, period, count, reset_at)
       VALUES ($1, $2, 'daily', 1, $3)
       ON CONFLICT (lead_id, period) 
       DO UPDATE SET count = rate_limits.count + 1, updated_at = datetime('now')
       WHERE rate_limits.reset_at > datetime('now')`, [(0, uuid_1.v4)(), leadId, dailyReset.toISOString()]);
        // Upsert weekly counter
        await database_1.default.query(`INSERT INTO rate_limits (id, lead_id, period, count, reset_at)
       VALUES ($1, $2, 'weekly', 1, $3)
       ON CONFLICT (lead_id, period) 
       DO UPDATE SET count = rate_limits.count + 1, updated_at = datetime('now')
       WHERE rate_limits.reset_at > datetime('now')`, [(0, uuid_1.v4)(), leadId, weeklyReset.toISOString()]);
    }
    /**
     * Check if current time is within business hours
     */
    async checkBusinessHours(action) {
    ...
    async processOptOut(leadId) {
        ...
        await database_1.default.query(`UPDATE leads SET opted_out = 1, updated_at = datetime('now') WHERE id = $1`, [leadId]);
        // Log action
        await database_1.default.query(`INSERT INTO action_log (id, lead_id, action_type, details) 
       VALUES ($1, $2, 'opt_out', $3)`, [(0, uuid_1.v4)(), leadId, JSON.stringify({ timestamp: new Date(), reason: 'User requested opt-out' })]);
    }
    /**
...
exports.default = new SafetyGuardrails();
