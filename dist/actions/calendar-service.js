...
class GoogleCalendarService {
    constructor() {
        this.oauth2Client = new googleapis_1.google.auth.OAuth2(config_1.config.googleClientId, config_1.config.googleClientSecret, 'urn:ietf:wg:oauth:2.0:oob');
     ...
     */
    async createBooking(lead, preferredTime) {
        try {
            // 1. Check for existing booking (double-book protection)
            const existingBooking = await database_1.default.query(`SELECT * FROM bookings 
         WHERE lead_id = $1 AND status = 'scheduled'`, [lead.id]);
            ...
            const event = await this.calendar.events.insert({
                calendarId: config_1.config.googleCalendarId,
                requestBody: {
            ...
            await database_1.default.query(`INSERT INTO bookings (id, lead_id, external_event_id, scheduled_at, duration_minutes, qualification_summary, status)
         VALUES ($1, $2, $3, $4, 30, $5, 'scheduled')`, [bookingId, lead.id, externalEventId, slot.start, qualificationSummary]);
            // 5. Update lead status
            await database_1.default.query(`UPDATE leads SET status = 'booked', updated_at = datetime('now') WHERE id = $1`, [lead.id]);
     ...
     */
    async findAvailableSlot(preferredTime) {
        const now = new Date();
        ...
        const response = await this.calendar.freebusy.query({
            requestBody: {
                ...
                items: [{ id: config_1.config.googleCalendarId }],
            },
        });
        const busySlots = response.data.calendars[config_1.config.googleCalendarId]?.busy || [];
     ...
     */
    searchForSlot(start, end, busySlots, preferredTime) {
     ...
     */
    async buildQualificationSummary(lead) {
        // Get latest LLM decision
        const result = await database_1.default.query(`SELECT * FROM llm_decisions 
       WHERE lead_id = $1 
...
Reasoning: ${decision.reasoning}`;
    ...
    async cancelBooking(bookingId) {
        try {
            const result = await database_1.default.query('SELECT * FROM bookings WHERE id = $1', [bookingId]);
            if (result.rows.length === 0) {
                return false;
            }
            ...
            // Update database
            await database_1.default.query(`UPDATE bookings SET status = 'cancelled', updated_at = datetime('now') WHERE id = $1`, [bookingId]);
            return true;
        }
        catch (error) {
