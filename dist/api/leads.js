...
router.post('/', async (req, res) => {
    try {
        ...
        await database_1.default.query(`INSERT INTO leads (id, email, name, company, source, status, created_at, updated_at, opted_out, followup_count)
             VALUES ($1, $2, $3, $4, 'manual', 'new', datetime('now'), datetime('now'), 0, 0)`, [id, email, name, company]);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const leadRes = await database_1.default.query('SELECT * FROM leads WHERE id = $1', [id]);
...
router.post('/:id/opt-out', async (req, res) => {
    try {
        const { id } = req.params;
        await database_1.default.query('UPDATE leads SET status = $1 WHERE id = $2', ['disqualified', id]);
...
router.post('/:id/refresh', async (req, res) => {
    try {
        ...
        await database_1.default.query('UPDATE leads SET updated_at = datetime("now") WHERE id = $1', [id]);
